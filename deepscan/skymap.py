...
def skymap(data, meshsize=100, nits=2, kappa=4, eps=5, thresh=0.5, 
           ...
           use_fft=False, getmask=False, interp_kwargs={}):
    '''
    ...
    for it in range(nits+1):
        ...
        #Increment the mask
        if it != 0:
            mask[:,:] = make_mask(data, sky, rms, kappa=kappa, eps=eps,
                                  thresh=thresh, use_fft=use_fft)
            mask|= np.isnan(sky) #Not needed after fill nans implementation
        
        #Do the sky estimate, updating the meshes
        measure_sky(data, mask=mask.astype(np.uint8), meshes=meshes)
